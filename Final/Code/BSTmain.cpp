...
// import all of the test data
	...
	while(index < 400)
	{
		...
		for(int i = 0; i < 100; i++)
		{
			...
			if(tree.searchKey(insertVal) == true) // if the value is found in the BST already
			{
			...
			{
				tree.addNode(insertVal); //always insert at the beginning to reduce insert time
				auto end = chrono::steady_clock::now();
				insertExecution = insertExecution + (double)chrono::duration_cast<chrono::nanoseconds>(end-start).count();
				countInsert++;
			}
